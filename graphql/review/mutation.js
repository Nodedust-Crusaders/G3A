...
const { addReviewHandler, removeReviewHandler } = require("../../handlers/reviews");
...
const reviewMutation = new GraphQLObjectType({
	...
	fields: {
		...
		addReviewToUser: {
			...
			resolve: async (source, { UserId, GameId, rating, comment}) => {
				const result = await addReviewHandler(
					UserId, 
					GameId,
					rating,
    ...
    // delete review from db. #ROLE only admins should be able to do this
        ...
        GameId: { type: new GraphQLNonNull(GraphQLID) },
			...
			resolve: async (source, { UserId, GameId }) => {
